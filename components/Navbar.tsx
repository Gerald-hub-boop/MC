...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-300 ${showSolidNav ? 'bg-white/95 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-5'
        }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-2 group cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
            <div className={`p-2 rounded-full transition-colors duration-300 ${showSolidNav ? 'bg-emerald-100 text-emerald-600' : 'bg-white text-emerald-600'
              }`}>
              <Mic className="w-6 h-6" />
            </div>
            <div className="flex flex-col">
              <span className={`font-bold text-lg leading-tight transition-colors duration-300 ${showSolidNav ? 'text-slate-900' : 'text-white'
                }`}>UKM MC</span>
              <span className={`text-xs font-medium tracking-wide transition-colors duration-300 ${showSolidNav ? 'text-emerald-600' : 'text-amber-300'
                }`}>UPNVJ</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <button onClick={() => handleNavClick('our-talent')} className={`text-sm font-medium transition-colors hover:text-amber-400 ${showSolidNav ? 'text-slate-600' : 'text-slate-200'}`}>Our Talent</button>


            <button onClick={() => handleNavClick('contact')}
              className={`inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-full transition-all shadow-lg cursor-pointer ${showSolidNav
          ...
          <div className="md:hidden">
            <button
              ...
              className={`p-2 rounded-md transition-colors ${showSolidNav ? 'text-slate-900 hover:bg-slate-100' : 'text-white hover:bg-white/10'
                }`}
            >
      ...
      {/* Mobile Menu Overlay */}
      <div className={`md:hidden absolute top-full left-0 w-full bg-white shadow-lg border-t border-slate-100 transition-all duration-300 origin-top transform ${isMenuOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0 hidden'
        }`}>
        <div className="flex flex-col px-4 py-6 space-y-4">
          ...
          <button onClick={() => handleNavClick('our-talent')} className="text-left text-slate-700 font-medium hover:text-emerald-600 transition-colors block p-2">Our Talent</button>
          <button onClick={() => handleNavClick('contact')}
